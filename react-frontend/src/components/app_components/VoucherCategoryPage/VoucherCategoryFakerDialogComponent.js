...
import voucherCategoryFakerFactory from "./voucherCategoryFakerFactory";
...
const VoucherCategoryFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = voucherCategoryFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("voucherCategory").create(elem));
...
export default connect(mapState, mapDispatch)(VoucherCategoryFakerDialogComponent);
