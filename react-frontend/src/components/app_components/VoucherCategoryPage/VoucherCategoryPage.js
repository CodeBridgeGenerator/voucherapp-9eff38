...
import VoucherCategoryDatatable from "./VoucherCategoryDataTable";
import VoucherCategoryEditDialogComponent from "./VoucherCategoryEditDialogComponent";
import VoucherCategoryCreateDialogComponent from "./VoucherCategoryCreateDialogComponent";
import VoucherCategoryFakerDialogComponent from "./VoucherCategoryFakerDialogComponent";
import VoucherCategorySeederDialogComponent from "./VoucherCategorySeederDialogComponent";
import SortMenu from "../../../services/SortMenu";
import FilterMenu from "../../../services/FilterMenu";

const VoucherCategoryPage = (props) => {
    ...
    const filename = "voucherCategory";
    ...
    useEffect(() => {
        const _getSchema = async () => {
            const _schema = await props.getSchema("voucherCategory");
    ...
    useEffect(() => {
        ...
        client
            ...
            .catch((error) => {
                ...
                setLoading(false);
                props.hide();
                props.alert({ title: "Voucher Category", type: "error", message: error.message || "Failed get Voucher Category" });
            });
  ...
  const onClickSaveHiddenfields = (ff) => {
    ...
    const deleteRow = async () => {
        try {
            await client.service("voucherCategory").remove(selectedEntityIndex);
            ...
            console.log({ error });
            props.alert({ title: "Voucher Category", type: "error", message: error.message || "Failed delete record" });
        }
    };
    ...
    const deleteAll = async () => {
      ...
      props.show();
        ...
        const promises = data.map((e) => client.service("voucherCategory").remove(e._id));
        await Promise.all(
          promises.map((p) =>
            p.catch((error) => {
              props.alert({
    ...
    const onRowClick = ({data}) => {
        ...
        navigate(`/app/voucherCategory/${data._id}`);
    };
...
rounded loading={loading} icon="pi pi-plus" onClick={() => setShowCreateDialog(true)} role="voucherCategory-add-button"/>
                    
                    </>
                </div>
            </div>
            <div className="grid align-items-center">
                <div className="col-12" role="voucherCategory-datatable">
                    <VoucherCategoryDatatable items={data} fields={fields} onRowDelete={onRowDelete} onEditRow={onEditRow} onRowClick={onRowClick} searchDialog={searchDialog} setSearchDialog={setSearchDialog}
...
/>            
      ...
      />
            ...
            <VoucherCategoryEditDialogComponent entity={_.find(data,{ _id : selectedEntityIndex})} show={showEditDialog} onHide={() => setShowEditDialog(false)} onEditResult={onEditResult} />
            <VoucherCategoryCreateDialogComponent entity={newRecord} onCreateResult={onCreateResult} show={showCreateDialog} onHide={() => setShowCreateDialog(false)}  />
            <VoucherCategoryFakerDialogComponent show={showFakerDialog} onHide={() => setShowFakerDialog(false)} onFakerCreateResults={onFakerCreateResults} />
            <VoucherCategorySeederDialogComponent show={showSeederDialog} onHide={() => setShowSeederDialog(false)} onSeederResults={onSeederResults} />
...
export default connect(mapState, mapDispatch)(VoucherCategoryPage);
