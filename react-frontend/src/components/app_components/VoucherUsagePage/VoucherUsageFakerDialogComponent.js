...
import voucherUsageFakerFactory from "./voucherUsageFakerFactory";
...
const VoucherUsageFakerDialogComponent = (props) => {
    ...
    useEffect(() => {
        ...
        client.service("voucher")
                        .find({ query: { $select: ["_id","voucherCode"] } })
                        .then((data) => {
                        setvoucherCodeIds(data.data);
                        });
    ...
    const onRun = async () => {
        let fakeData = voucherUsageFakerFactory(props.user,fakerCount,voucherCodeIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("voucherUsage").create(elem));
...
export default connect(mapState, mapDispatch)(VoucherUsageFakerDialogComponent);
