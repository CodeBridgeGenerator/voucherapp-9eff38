...
import voucherReportFakerFactory from "./voucherReportFakerFactory";
...
const VoucherReportFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = voucherReportFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("voucherReport").create(elem));
...
export default connect(mapState, mapDispatch)(VoucherReportFakerDialogComponent);
