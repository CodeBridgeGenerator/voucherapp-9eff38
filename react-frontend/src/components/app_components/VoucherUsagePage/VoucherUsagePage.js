...
import VoucherUsageDatatable from "./VoucherUsageDataTable";
import VoucherUsageEditDialogComponent from "./VoucherUsageEditDialogComponent";
import VoucherUsageCreateDialogComponent from "./VoucherUsageCreateDialogComponent";
import VoucherUsageFakerDialogComponent from "./VoucherUsageFakerDialogComponent";
import VoucherUsageSeederDialogComponent from "./VoucherUsageSeederDialogComponent";
import SortMenu from "../../../services/SortMenu";
import FilterMenu from "../../../services/FilterMenu";

const VoucherUsagePage = (props) => {
    ...
    useEffect(() => {
        const _getSchema = async () => {
            const _schema = await props.getSchema("voucherUsage");
    ...
    useEffect(() => {
        ...
        client
            ...
            .find({ query: { $limit: 10000 , voucherCode : urlParams.singleVoucherId  , $populate : [
                {
                path: "createdBy",
                service: "users",
                select: ["name"],
            ...
            .catch((error) => {
                ...
                setLoading(false);
                props.hide();
                props.alert({ title: "Voucher Usage", type: "error", message: error.message || "Failed get Voucher Usage" });
            });
  ...
  const onClickSaveHiddenfields = (ff) => {
    ...
    const deleteRow = async () => {
        try {
            await client.service("voucherUsage").remove(selectedEntityIndex);
            ...
            console.log({ error });
            props.alert({ title: "Voucher Usage", type: "error", message: error.message || "Failed delete record" });
        }
    };
    ...
    const deleteAll = async () => {
      ...
      props.show();
        ...
        const promises = data.map((e) => client.service("voucherUsage").remove(e._id));
        await Promise.all(
          promises.map((p) =>
            p.catch((error) => {
              props.alert({
    ...
    const onRowClick = ({data}) => {
        ...
        navigate(`/app/voucherUsage/${data._id}`);
    };
...
rounded loading={loading} icon="pi pi-plus" onClick={() => setShowCreateDialog(true)} role="voucherUsage-add-button"/>
                    
                    </>
                </div>
            </div>
            <div className="grid align-items-center">
                <div className="col-12" role="voucherUsage-datatable">
                    <VoucherUsageDatatable items={data} fields={fields} onRowDelete={onRowDelete} onEditRow={onEditRow} onRowClick={onRowClick} searchDialog={searchDialog} setSearchDialog={setSearchDialog}
...
/>            
      ...
      />
            ...
            <VoucherUsageEditDialogComponent entity={_.find(data,{ _id : selectedEntityIndex})} show={showEditDialog} onHide={() => setShowEditDialog(false)} onEditResult={onEditResult} />
            <VoucherUsageCreateDialogComponent entity={newRecord} onCreateResult={onCreateResult} show={showCreateDialog} onHide={() => setShowCreateDialog(false)}  />
            <VoucherUsageFakerDialogComponent show={showFakerDialog} onHide={() => setShowFakerDialog(false)} onFakerCreateResults={onFakerCreateResults} />
            <VoucherUsageSeederDialogComponent show={showSeederDialog} onHide={() => setShowSeederDialog(false)} onSeederResults={onSeederResults} />
...
export default connect(mapState, mapDispatch)(VoucherUsagePage);
